...
case class Cube(x: Int, y: Int, z: Int, w: Int = 0, active: Boolean = false) {
  ...
  def neighbors(others: List[Cube]) = {
    others.filter {
      c =>
      areClose(c.x, x) && areClose(c.y, y) && areClose(c.z, z) && areClose(c.w, w) &&
  ...
  override def equals(other: Any): Boolean = other match {
    case other: Cube => other.canEqual(this) && x == other.x && y == other.y && z == other.z && w == other.w
    case _ => false
  }
...
object Day17 {
  def main(args: Array[String]) {
    ...
    def doCycles(cubeList: List[Cube], secondStar: Boolean = false, cycle: Int = 0, maxCycle: Int = 6): List[Cube] = {
      if (cycle == maxCycle)
        cubeList
      else {
